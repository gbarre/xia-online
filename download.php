	...
	if (!empty($_GET['token']) && !empty($_GET['id'])) {
		...
		if ($can_download == true) {
			if (!isset($_GET['download'])) {
				...
				// Xia Extend : no log for xia download yet
				if (!isset($_GET['download_xe'])) {
				...
				// DOWNLOAD
				$theme = $_GET['download_xe'];
				$replace = '_' . $theme . '.html';
			  $random_compiled_file_name = preg_replace('/\.svg/', $replace, $got_url['url']);
				$compiled_file_name = preg_replace('/\.svg/', $replace, $real_file_url);


				$real_file = 'compiled/'.basename($compiled_file_name);
				$random_file = realpath('compiled/'.basename($random_compiled_file_name));
			}
			// Close Xia Extend : no log for xia download yet
...
<div class="col-xs-12 col-sm-12 col-lg-6 col-lg-offset-3">
			...
			<?php
				...
				?>
					<div class="text-center">
						...
						<!-- Xia extend -->
						<?php include_once('./xe_download.php'); ?>
						<!-- Close Xia extend -->
					</div>
				<?php
				}
	...
	<div class="login_form_links">
		...
		<!-- Xia extend -->
		<p><a href="./public.php">Retour aux ressources publiques.</a></p>
		<!-- Close Xia extend -->
	</div>
</div>
